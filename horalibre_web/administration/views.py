...
from .forms import ProfessionalForm, ProfessionalListForm, SecretaryForm, PatientForm, CaseForm, PatientListForm, SecretaryListForm, CaseListForm
...
def create_professional(request):
    # from Django
    # user = create_user(username, email=None, password=None, **extra_fields)
    # username, 150 chars or fewer
    # password, 50 chars or fewer
    # user.FIELD = 'VALUE'
    # first_name   30 chars or fewer
    # last_name    30 chars or fewer
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if entered_username is None:
                    ...
                    error_message = "El nombre de usuario ingresado ya existe, elija otro"
                    return render(request, 'administration/create_professional.html',
                                 {'form': form, 'error_message': error_message})
        # if a GET (or any other method) we'll create a blank form
        else:
            form = ProfessionalForm()
        return render(request, 'administration/create_professional.html', {'form': form})
    else:
        return render(request, 'login/login.html')
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            pre_data = {'email' : proff.user.email, 
                        'first_name' : proff.user.first_name, 'last_name' : proff.user.last_name,
                        'dni': proff.dni, 'phone_number': proff.phone_number,
...
def create_secretary(request):
    if request.user.is_authenticated:
        ...
        # if a GET (or any other method) we'll create a blank form
            ...
            form = SecretaryForm()
        return render(request, 'administration/create_secretary.html', {'form': form})
    else:
        return render(request, 'login/login.html')
...
def create_patient(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if entered_dni is None:
                    ...
                    pre_data = { 'first_name' : first_name, 'last_name' : last_name,
                            'health_insurance' : health_insurance , 'birthdate' : birthdate,
                            'phone_number': phone_number}
                    form = PatientForm(initial=pre_data)
                    error_message = "El DNI del paciente ingresado ya existe en el sistema."
        ...
        return render(request, 'administration/create_patient.html', {'form': form})
    else:
        return render(request, 'login/login.html')
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                entered_case = Case.objects.filter(patient=patient, professional=professional, coordinator=coordinator).first()
        ...
        # if a GET (or any other method) we'll create a blank form
            ...
            form = CaseForm()
        return render(request, 'administration/create_case.html', {'form': form})
    else:
        return render(request, 'login/login.html')
